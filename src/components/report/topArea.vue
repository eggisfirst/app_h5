...
import { getCityByDealerBoss } from "@/api/report";
export default {
  data() {
    return {
      isShow: false,
  ...
  computed: {
    ...mapState({
      areaLine: state => state.report.areaLine,
  ...
  methods: {
    ...
    // 获取区域信息
    getAreaList() {
      getCityByDealerBoss(this.url).then(res => {
        let { data, status } = res;
        if (status) {
          this.areaList = data;
          ...
          if (this.typename == "Store Manager") {
            ...
            this.$emit("getArea", data[0].shopId);
            // }
          } else {
            // if (!this.areaMsg) {
            this.setAreaName(data[0]);
            this.$emit("getArea", data[0]);
            // }
          }
